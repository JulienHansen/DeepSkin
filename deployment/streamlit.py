...
def get_metrics(metric_filter: str, metric_label: str, interval_seconds: int):
    # Définit l'intervalle de temps de la requête en fonction de l'interval sélectionné
    interval = monitoring_v3.TimeInterval(
        start_time={"seconds": int(time.time()) - interval_seconds},
        end_time={"seconds": int(time.time())}
    )
    
    # Crée une requête de métrique
    ...
    # Récupère les séries temporelles (les métriques)
    ...
    for series in time_series:
        for point in series.points:
            ...
            if(metric_label == "CPU" or metric_label == "RAM" or metric_label == "Request Latency"):
                ...
                start_seconds = start_time.timestamp()  # Converti en secondes
            ...
            if(metric_label == "Request Count"):
                timestamp = point.interval.end_time
                value = point.value.int64_value  # La valeur de la métrique est un entier
